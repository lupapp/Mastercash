...
/*================================*/
	...
	function nz_colorbox( $atts, $content = null ) {
	    ...
	    if(empty($width)){
	    	...
	    	$style .= 'width:'.absint($width).'px;max-width:'.absint($width).'px;';
	    }
...
/*================================*/
	...
	function nz_testimonials($atts, $content = null) {
		...
		$output = "";
		$output .= '<div id="nz-testimonials-'.$id_counter.'" data-columns="'.absint($columns).'" data-autoplay="'.esc_attr($autoplay).'" class="'.sanitize_html_class($animate).' nz-testimonials lazy">';
			$output .= do_shortcode($content);
		$output .= '</div>';
...
/*================================*/
	...
	function nz_cl($atts, $content = null) {
		...
		$output .= '<div id="nz-clients-'.$id_counter.'" class="lazy nz-clients '.sanitize_html_class($animate).'" data-columns="'.absint($columns).'" data-autoplay="'.esc_attr($autoplay).'">';
			$output .= do_shortcode($content);
		$output .= '</div>';
...
/*================================*/
	...
	function nz_carousel($atts, $content = null) {
		...
		$output = "";
		$output .= '<div id="nz-carousel-'.$id_counter.'" class="lazy nz-carousel '.sanitize_html_class($animate).'" data-autoplay="'.esc_attr($autoplay).'" data-columns="'.absint($columns).'">'.do_shortcode($content).'</div>';
...
/*================================*/
	...
	function nz_ss($atts, $content = null) {
		...
		$output .='<div id="nz-ss-'.$id_counter.'" class="lazy nz-ss" data-autoplay="'.esc_attr($autoplay).'" data-bullets="'.esc_attr($bullets).'" data-nav="'.esc_attr($nav).'">'. $nav_color.do_shortcode($content).'</div>';
	...
	function nz_rposts($atts, $content = null) {
		...
		$recent_posts = new WP_Query(array( 'orderby' => 'date', 'posts_per_page' => absint($posts_number), 'cat' => $cat,'post_status' => 'publish','ignore_sticky_posts' => true));
			...
			if($recent_posts->have_posts()){
				...
				$output .= '<div id="nz-recent-posts-'.$id_counter.'" data-animate="'.esc_attr($animate).'" data-autoplay="'.esc_attr($autoplay).'" data-columns="'.absint($columns).'" class="lazy nz-recent-posts '.sanitize_html_class($version).' '.sanitize_html_class($nz_ninzio_grid).' nz-clearfix">';
	...
	function nz_rportfolio($atts, $content = null) {
		...
		$recent_portfolio = new WP_Query($recent_query_opt);
			...
			if($recent_portfolio->have_posts()){
					...
					$output .= '<div id="nz-recent-portfolio-'.$id_counter.'" data-animate="'.esc_attr($animate).'" data-autoplay="'.esc_attr($autoplay).'" data-columns="'.absint($columns).'" class="lazy nz-recent-portfolio '.$classes.'">';
						...
						if ($filter == "true" && $version != "carousel") {
							...
							$output .= '<div class="nz-portfolio-filter">';
								$output .= '<span class="active filter" data-group="all">'.__('Show All', TEMPNAME).'</span>';
