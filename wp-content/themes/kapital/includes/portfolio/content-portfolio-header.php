<?php  
    ...
    if (is_single()) {
        ...
        $nz_rh_height           = (isset( $values['rh_height'][0]) && !empty($values['rh_height'][0])) ? $values["rh_height"][0] : "115";
        ...
        if ($nz_parallax == "true") {
            ...
            $nz_back_img_position   = "center_top";
            $nz_back_img_attachment = "fixed";
        ...
        $nz_rh_height           = (isset($nz_ninzio['port-rh-height']) && $nz_ninzio['port-rh-height']) ? esc_attr($nz_ninzio["port-rh-height"]) : "115";
        ...
        $nz_back_img_position   = (isset($nz_ninzio['port-back']['background-position']) && $nz_ninzio['port-back']['background-position']) ? $nz_ninzio['port-back']['background-position'] : "left top";
...
<?php if (!is_single() && !is_tax('portfolio-category') && $nz_filter == "true"): ?>
	...
	?>
	<div class="nz-portfolio-filter width-<?php echo $nz_port_width; ?> portfolio-archive-filter">
        <div class="container">
    		<span class="active filter" data-group="all"><?php echo __('Show All', TEMPNAME); ?></span>
    		<?php foreach(get_terms('portfolio-category',$args) as $filter_term) { ?>
    			<span class="filter" data-group="<?php echo $filter_term->slug; ?>"><?php echo $filter_term->name ?></span>
    		<?php } ?>
        </div>
	</div>
<?php endif ?>